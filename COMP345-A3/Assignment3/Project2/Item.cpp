...
bool itemCheck(Item &i) {
	...
	if (i.type == "helmet") {
		for (int j = 0; j < b; j++)
			if (i.bonus[j] == "INT" || i.bonus[j] == "WISDOM" || i.bonus[j] == "ARMORCLASS")
				return true;
	}
	...
	if (i.type == "ring") {
		for (int j = 0; j < b; j++)
			if (i.bonus[j] == "ARMORCLASS" || i.bonus[j] == "STR" || i.bonus[j] == "CONSTITUTION" || i.bonus[j] == "WISDOM" || i.bonus[j] == "CHARISMA")
				return true;
	}

	if (i.type == "belt") {
		for (int j = 0; j < b; j++)
			if (i.bonus[j] == "STR" || i.bonus[j] == "CONSTITUTION")
				return true;
	}
...
void Item::addToBackpack()
{
	backpack.v.push_back(this);
}
void Item::displayBackpack()
{
	displayInventory(backpack);
