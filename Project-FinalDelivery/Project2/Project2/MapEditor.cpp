...
void MapEditor::createCampaign(Campaign &c) {
	...
	if (choice == 1) {
		...
		cin >> choice;
		//	c./*numberOfMaps*/ += 1;
	//	c.mapNames.push_back("spawn.txt");
		
			
			if (choice == 1) {
				do {
					maps.push_back(Map());
					cout << "What is the name of the map? (ex:forest, dungeon, etc)" << endl;
					cin >> name;
					//c.mapNames.push_back(name);
					maps[numberOfMaps].name = name;
					me.createMap(maps[numberOfMaps]);
					c.addMaptoCampaign(name, maps[numberOfMaps]);
					cout << c.mapOfGameMaps.find(name)->second << endl;
					numberOfMaps++;
					cout << "Press 1 to add another map or press 2 to save the campaign." << endl;
					cin >> choice;
				} while (choice == 1);
			}
		...
		if (choice == 2) {
			//cout << "Do you want to save the campaign? Press 1 to save or 2 to exit." << endl;
			//cin >> choice;
			//if (choice == 1) {
				c.saveCampaign();
				return;
